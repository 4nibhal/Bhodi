...
from bodhi_rag.interfaces.cli.main import _health_command, main as cli_main
...
class TestCliMain:
    ...
    def test_index_help(self, capsys):
        ...
        with pytest.raises(SystemExit) as exc_info:
            with patch.object(sys, "argv", ["bodhi-rag", "index", "--help"]):
                cli_main()
    ...
    def test_query_help(self, capsys):
        ...
        with pytest.raises(SystemExit) as exc_info:
            with patch.object(sys, "argv", ["bodhi-rag", "query", "--help"]):
                cli_main()
        assert exc_info.value.code == 0
        captured = capsys.readouterr()
        assert "usage:" in captured.out
