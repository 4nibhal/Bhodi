...
class BudgetValidator:
    ...
    def validate_retrieval(
        ...
        if pollution_rate > thresholds.pollution_rate_max:
            failures.append(
                f"pollution_rate {pollution_rate:.3f} > "
                f"threshold {thresholds.pollution_rate_max:.3f}",
            )
        ...
        if memory_leak_rate > thresholds.memory_leak_rate_max:
            failures.append(
                f"memory_leak_rate {memory_leak_rate:.3f} > "
                f"threshold {thresholds.memory_leak_rate_max:.3f}",
            )
